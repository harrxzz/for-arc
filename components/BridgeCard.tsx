      ...
      <div className={`tx-card-shell rounded-3xl p-5 ${isDark ? 'glass-arc' : glassCard} hover-glow`}>
        ...
        <div className={`rounded-2xl p-4 mb-2 tx-input-panel ${glassInput}`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-arc-arc ${
                  isDark
                  ...
                  >
                    {BRIDGE_SOURCE_CHAINS.map(chain => (
                      <button
                        ...
                        } ${sourceChain.id === chain.id ? isDark ? 'bg-white/8 text-arc-arc' : 'bg-white/5 text-arc-arc font-medium' : ''}`}
                      >
          ...
          <div className="flex items-center gap-3">
            <div className={`flex items-center gap-1.5 px-3 py-2 rounded-xl flex-shrink-0 ${
              ...
              <span className={`text-sm font-semibold ${isDark ? 'text-arc-arc' : 'text-arc-arc'}`}>USDC</span>
            </div>
            <input
        ...
        <div className="flex justify-center my-2">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center pulse-glow ${
            isDark
              ? 'bg-white/8 border border-white/10 text-arc-arc'
              : 'bg-white/90 border border-white/90 shadow-sm text-arc-arc'
          }`}>
        ...
        <div className={`rounded-2xl p-4 mb-4 tx-input-panel ${glassInput}`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border ${
              isDark ? 'bg-arc-arc/10 border-arc-arc/20' : 'bg-white/5 border-arc-arc'
            }`}>
              <div className="w-1.5 h-1.5 rounded-full bg-arc-arc animate-pulse" aria-hidden="true" />
              <span className="text-xs font-medium text-arc-arc">Arc Testnet</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className={`flex items-center gap-1.5 px-3 py-2 rounded-xl flex-shrink-0 ${
        ...
        <p className={`text-center text-xs mt-3 ${muted}`}>
          ...
          <a href="https://www.circle.com/cross-chain-transfer-protocol" target="_blank" rel="noopener noreferrer"
            className="text-arc-arc hover:underline"
          >
            Circle CCTP
          </a>
        </p>
