  ...
  return (
    <div className={`rounded-2xl flex flex-col ${card}`} style={{ height: '600px' }}>
      ...
      <div className="flex items-center gap-3 p-4 border-b border-white/8">
        <div className="w-9 h-9 rounded-xl bg-arc-light/15 flex items-center justify-center">
          <Bot size={18} className="text-arc-light" />
        </div>
        <div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
              msg.role === 'agent' ? 'bg-arc-light/15' : 'bg-white/10'
            }`}>
              {msg.role === 'agent'
                ? <Bot size={13} className="text-arc-light" />
                : <User size={13} className="text-white" />
              }
            </div>
            <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm whitespace-pre-wrap ${
              msg.role === 'agent' ? 'bg-white/5 text-slate-200' : 'bg-arc-light/15 text-white'
            }`}>
              {msg.content}
              {msg.txHash && (
                <a href={`https://testnet.arcscan.app/tx/${msg.txHash}`}
                  target="_blank" rel="noopener noreferrer"
                  className="block mt-2 text-xs text-arc-light underline hover:opacity-80">
                  View on ArcScan →
                </a>
              )}
        ...
        {loading && (
          <div className="flex gap-3">
            <div className="w-7 h-7 rounded-lg bg-arc-light/15 flex items-center justify-center">
              <Bot size={13} className="text-arc-light" />
            </div>
      ...
      {pendingIntent && !loading && (
        <div className="px-4 pb-2 flex gap-2">
          <button onClick={() => sendMessage('confirm')}
            className="flex-1 py-2 rounded-xl bg-arc-light/20 border border-arc-light/30 text-arc-light text-xs font-semibold hover:bg-arc-light/30 transition-colors">
            ✓ Confirm
          </button>
      ...
      <div className="p-4 border-t border-white/8">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-slate-600 outline-none focus:border-arc-light/40 transition-colors"
          />
          <button onClick={() => sendMessage()} disabled={!input.trim() || loading}
            className="w-10 h-10 rounded-xl bg-arc-light/15 flex items-center justify-center hover:bg-arc-light/25 transition-colors disabled:opacity-40">
            <Send size={14} className="text-arc-light" />
          </button>
        </div>
      </div>
    </div>
